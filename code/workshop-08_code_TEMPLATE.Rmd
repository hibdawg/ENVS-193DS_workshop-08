...
drought_exp_clean <- drought_exp |> # INSERT COMMENT HERE
  ...
  # make a column of full drought species names
  mutate(water_treatment = case_match(
    water,
    "WW" ~ "Well watered",
    "DS" ~ "Drought stressed"
  )) |> 
  # Make sure that the species name is a factor 
  mutate(species_name = as_factor(species_name),
         species_name = fct_relevel(species_name,
                                    ...
                                    "Grindelia camporum")) |> 
  # ensure that the water treatment is a factor and order those
...
slice_sample(
  ...
  n = 10 # specificing twn rows to display
)
```
...
```{r water-treatment, fig.width=10}
# base layer: ggplot
ggplot(data = drought_exp_clean,
...
```{r final-figure, fig.height=4, fig.width=12}
# base layer: ggplot
